...
onBeforeMount(() => {
  ...
  if (colorMode.preference === 'system') {
    try {
      ...
      console.log('[ColorMode] Reset from system → light')
    } catch (e) {
      console.warn('[ColorMode] Failed to reset color mode:', e)
    }
  }
})

const sendEventChangeTheme = (key: string) => {
  $bus.emit('change-theme', key)
}
</script>
    ...
    >
      ...
      >
        <HeadlessListboxOption
          ...
          @click="sendEventChangeTheme(theme.key)"
        >
