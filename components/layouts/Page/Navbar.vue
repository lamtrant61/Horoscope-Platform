...
const userAmount = ref<null | number>(null)
...
onMounted(async () => {
  ...
  if (user) {
    isLogin.value = true
    const userBalance: any = await callGetUserBalance()
    if (userBalance.error_code) return
    userAmount.value = userBalance.data.balance
  }
})
// drawer
...
<template>
  <header
    class="navbar-div flex fixed backdrop-filter backdrop-blur-md top-0 z-40 w-full max-w-[100vw] flex-none transition-colors duration-300 lg:z-50 border-b border-gray-950/10 dark:border-gray-50/[0.2] bg-white/[0.5] dark:bg-gray-950/[0.5]"
  >
    <!-- content -->
    <div
      ...
      <div>
        <slot name="title">
          <NuxtLink to="/" class="font-bold text-lg text-primary-500">
            <img src="/logo.png" class="w-9 h-9 mr-2 inline-block" alt="Logo" />
            <span class="capitalize text-sm md:text-xl">{{
              awesome.name
            }}</span>
          </NuxtLink>
        </slot>
      ...
      >
        <div class="flex space-x-4 text-sm items-center">
          ...
          <el-text
            ...
            tag="b"
            type="primary"
            >{{ `Số dư: ${userAmount.toLocaleString('vi-VN')}đ` }}</el-text
          >
              ...
              <template v-if="item?.type === 'button'">
                <AwesomeButton
                  ...
                  :to="
                    item.to.url
                      ? { path: item.to.url }
                      : parseMenuRoute(item.to)
                  "
                  class="w-full"
                />
                <AwesomeButton
                  v-if="item?.to.needLogin && isLogin"
                  :key="i"
                  :text="parseMenuTitle(item?.title)"
                  size="sm"
                  :to="
...
<style scoped>
.navbar-div .el-text.el-text--primary {
