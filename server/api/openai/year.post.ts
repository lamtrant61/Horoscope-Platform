...
import { horoscopeBotWithYear } from '../../utils/openai'
...
export default defineEventHandler(async (event) => {
  const costHoroscope = parseInt(process.env.COST_YEAR_HOROSCOPE || '50000')
  ...
  if (checkSolveHoroscope) {
    ...
    dataSolveHoroscope = await horoscopeBotWithYear(astrolabe)
    console.log(
      dataSolveHoroscope,
      '\n----------\nLuận giải lá số theo năm thành công!!!!!',
    )
